...
def update_rotation(target_value, current_angle, velocity, multiplier, start_angle, offset=0):
    ...
    target_angle = target_angle % 360  # Normalize to 0-360 degrees

    # Calculate the difference to reach target angle
    diff = target_angle - current_angle

    # Determine the shortest rotation direction (clockwise or counterclockwise)
    if diff > 180:
        diff -= 360
    elif diff < -180:
        diff += 360

    # Update velocity based on the difference
    ...
    # Print for debugging
    print(f"Target: {target_angle}, Current: {current_angle}, Velocity: {velocity}")

    return current_angle, velocity

# Pygame loop for displaying the indicators
def display_loop():
    global hr_angle, power_angle, hr_velocity, power_velocity
    ...
    while running:
        ...
        hr_angle, hr_velocity = update_rotation(target_hr_value, hr_angle, hr_velocity,
                                                hr_multiplier, hr_start_angle, hr_offset)
        power_angle, power_velocity = update_rotation(target_power_value, power_angle, power_velocity,
