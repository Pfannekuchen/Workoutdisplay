...
def update_rotation(target_value, prev_value, current_angle, velocity, multiplier, start_angle, offset=0):
    ...
    # Damping factor to prevent overshooting and slow down near the target
    damping = 0.1  # Adjust this value to control how much the velocity slows down
    if abs(diff) < 10:  # Small threshold to stop once close to target
        velocity *= damping  # Apply damping to velocity to slow it down

    # Apply the acceleration based on the difference
    acceleration = max_acceleration * abs(diff)

    # Adjust velocity based on direction of movement
    if diff > 0:
...
def display_loop():
    ...
    while running:
        ...
        hr_angle, hr_velocity = update_rotation(heart_rate, prev_hr_value, hr_angle, hr_velocity,
                                                hr_multiplier, hr_start_angle, hr_offset)
        power_angle, power_velocity = update_rotation(power, prev_power_value, power_angle, power_velocity,
